...
class PlaylistPreviewView(LoginRequiredMixin, DetailView):
    ...
    def _calculate_auto_duration(self, item):
        ...
        Uses store_filter_id to distinguish screen types:
        - With store_filter_id: KPI/per-employee screen → duration based on employee count
        - Without store_filter_id: Company-wide leaderboard → duration based on store page count
        """
        ...
        if screen.page_duration:
            if screen.store_filter_id:
                # Store-filtered screen (e.g., Staff KPI Card) — use employee count
                try:
                ...
                # Company-wide screen (e.g., Leaderboard) — use store page count
                try:
